...
#check Y
check_Y_factor = function (Y) {
	#check response error
	#return Y levels number after triming
	y.table = table(Y)
	if (sum(y.table <= 1) > 0 ) {
		e.i = which(y.table <= 1)
		if (length(e.i) > 1) {
...
for (j in 1:bootstrap.num) 
{
	...
	repeat.num = 0;
	while (y.num < 2) {
		my.seed = all.seed[j] + repeat.num * all.seed[j]
		set.seed(my.seed)
		sample.i = sample(1:nrow(myX), sample.num, replace=T)
		repeat.num = repeat.num + 1
		message (paste0("Bootstrap of sample index, repeat No. ", repeat.num))
		y.num = check_Y_factor(myY[sample.i])
		if (repeat.num > 20) stop ("Levels of Y is less than 2, and Cannot find a correct sample.i")
	}
