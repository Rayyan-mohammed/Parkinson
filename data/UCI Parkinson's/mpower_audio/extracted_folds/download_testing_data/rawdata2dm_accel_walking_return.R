...
####
testing <- merge(testing,testing_json_files, by.x="accel_walking_return.json.items", by.y="testing_json_fileId", all.x=TRUE, all.y=F)
...
#walking_json_file = testing[1,16]
...
accel_walking_return.json.items_testing.features <- walkFeatures.df %>% filter(idx %in% selected_records)

save(accel_walking_return.json.items_testing.features, file="accel_walking_return.json.items_testing.features.rdata")
