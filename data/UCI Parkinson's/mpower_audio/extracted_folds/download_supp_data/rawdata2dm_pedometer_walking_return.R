...
####
supp_training <- merge(supp_training,supp_training_json_files, by.x="pedometer_walking_return.json.items", by.y="supp_training_json_fileId", all.x=TRUE, all.y=F)
...
str(dat)
dat.ncol <- length(names(unlist(dat[1,])))
...
walkFeatures <-
	dlply(
		  ...
		  #.variables = c("idx", "pedometer_walking_return.json.items", "supp_training_json_file"),
		  .variables = ("idx"),
		  .fun = function(row) {
...
pedometer_walking_return.json.items_supp.features <- walkFeatures.df %>% filter(idx %in% selected_records)

save(pedometer_walking_return.json.items_supp.features, file="pedometer_walking_return.json.items_supp.features.rdata")
