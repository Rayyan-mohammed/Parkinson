...
####
supp_training <- merge(supp_training,supp_training_json_files, by.x="deviceMotion_walking_rest.json.items", by.y="supp_training_json_fileId", all.x=TRUE, all.y=F)
...
str(walkFeatures.df)
#100117749 obs. of  19 variables:

# Only keep the non-redundant data
deviceMotion_walking_rest.json.items_supp.features <- walkFeatures.df %>% filter(idx %in% selected_records)

save(deviceMotion_walking_rest.json.items_supp.features, file="deviceMotion_walking_rest.json.items_supp.features.rdata")
