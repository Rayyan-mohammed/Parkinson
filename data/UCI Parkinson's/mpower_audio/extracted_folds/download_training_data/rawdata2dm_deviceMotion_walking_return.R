...
####
actv_walking <- merge(actv_walking,outbound_Walking_json_files, by.x="deviceMotion_walking_return.json.items", by.y="outbound_Walking_json_fileId", all.x=TRUE, all.y=F)
...
if (F) {
w = list()
for (i in 1:nrow(actv_walking))
{
	cat (actv_walking[i, "outbound_Walking_json_file"], "......\n")
	out = my.getWalkFeatures(actv_walking[i, "outbound_Walking_json_file"])
	w[[length(w)+1]] = out
}
}
...
deviceMotion_walking_return.json.items.features <- walkFeatures.df %>% filter(idx %in% selected_records)

save(deviceMotion_walking_return.json.items.features, file="deviceMotion_walking_return.json.items.features.rdata")
