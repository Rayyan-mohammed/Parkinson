...
####
actv_walking <- merge(actv_walking,outbound_Walking_json_files, by.x="pedometer_walking_return.json.items", by.y="outbound_Walking_json_fileId", all.x=TRUE, all.y=F)
...
walkFeatures <-
	dlply(
		  ...
		  #.variables = c("idx", "pedometer_walking_return.json.items", "outbound_Walking_json_file"),
		  .variables = ("idx"),
		  .fun = function(row) {
...
pedometer_walking_return.json.items.features <- walkFeatures.df %>% filter(idx %in% selected_records)

save(pedometer_walking_return.json.items.features, file="pedometer_walking_return.json.items.features.rdata")
