...
####
testing <- merge(testing,testing_json_files, by.x="pedometer_walking_return.json.items", by.y="testing_json_fileId", all.x=TRUE, all.y=F)
...
walkFeatures <-
	dlply(
		  ...
		  #.variables = c("idx", "pedometer_walking_return.json.items", "testing_json_file"),
		  .variables = ("idx"),
		  .fun = function(row) {
...
pedometer_walking_return.json.items_testing.features <- walkFeatures.df %>% filter(idx %in% selected_records)

save(pedometer_walking_return.json.items_testing.features, file="pedometer_walking_return.json.items_testing.features.rdata")
