...
####
supp_training <- merge(supp_training,supp_training_json_files, by.x="accel_walking_outbound.json.items", by.y="supp_training_json_fileId", all.x=TRUE, all.y=F)
...
accel_walking_outbound.json.items_supp.features <- walkFeatures.df %>% filter(idx %in% selected_records)

save(accel_walking_outbound.json.items_supp.features, file="accel_walking_outbound.json.items_supp.features.rdata")
