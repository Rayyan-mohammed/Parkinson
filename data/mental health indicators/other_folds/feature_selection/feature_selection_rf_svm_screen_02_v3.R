...
if (sum(is.na(match(ml.algorithm, all.ml.algorithm))) > 0) {
	message ("Some ml.algorithm are not one of all.ml.algorithm")
}
...
if (!is.null(snp.rank.f)) {
	if (!file.exists(snp.rank.f)) {
		stop (paste0("Ranking file ", snp.rank.f, " is not NULL but not exist."))
	}
...
for (j in 1:n) 
{
	message (paste0("For ", j , " out of ", n, " predictor number: ", snp.num[j], " ... "))
